...
## Next Steps
1. Review and approve this plan again
2. Begin with Phase 1: Security & Stability fixes
3. Proceed phase by phase, tracking progress in issues/PRs
4. see below a little guide to assist if needed
5. use mpc tool context 7 when needed and follow rules regading
---

*This plan is a living document and should be updated as development progresses and requirements evolve.* 


**** just a little guide to follow inlline with your plan more like a guard rail ******
...
* **Integrate Azure Document Intelligence** for OCR/data extraction
* **Implement Azure Cognitive Search** for smart document search
* **Build Zero-Hallucination RAG Pipeline for "Ask Esus":**
    * **Core Principle:** The AI must be strictly grounded in the retrieved document context. It should never invent, infer, or use outside knowledge.
    * **Guardrail Implementation:** The prompt sent to the AI must explicitly command it to refuse to answer if the context is insufficient.
    * **Confidence scoring** for all extracted data
    ```ts
    // Pseudocode for the Guarded RAG Prompt
    function buildGuardedPrompt(query, retrievedChunks) {
      const context = retrievedChunks.map(c => `Source: ${c.sourceDocument}, Content: ${c.textContent}`).join('\n---\n');
    
      return `
        You are a precise audit assistant. Answer the user's query based ONLY on the provided context below.
    ...
    // Main RAG Logic
    const chunks = await azureCognitiveSearch(query, projectId);
    const guardedPrompt = buildGuardedPrompt(query, chunks);
    const answer = await openai.generate({ prompt: guardedPrompt });
    ```

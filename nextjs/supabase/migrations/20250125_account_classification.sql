...
CREATE INDEX idx_account_mappings_org_id ON account_mappings(organization_id);
CREATE INDEX idx_account_mappings_account_code ON account_mappings(account_code);
CREATE INDEX idx_account_mappings_account_type ON account_mappings(account_type);
CREATE INDEX idx_account_mappings_category ON account_mappings(category);
CREATE INDEX idx_account_mappings_active ON account_mappings(is_active);
CREATE INDEX idx_account_mappings_ifrs ON account_mappings USING GIN (ifrs_mapping);
CREATE INDEX idx_account_mappings_gaap ON account_mappings USING GIN (gaap_mapping);
CREATE INDEX idx_account_mappings_frs ON account_mappings USING GIN (frs_mapping);

CREATE INDEX idx_templates_org_id ON chart_of_accounts_templates(organization_id);
CREATE INDEX idx_templates_industry ON chart_of_accounts_templates(industry_type);

CREATE INDEX idx_categorization_rules_org_id ON account_categorization_rules(organization_id);
CREATE INDEX idx_categorization_rules_priority ON account_categorization_rules(priority DESC);
CREATE INDEX idx_categorization_rules_active ON account_categorization_rules(is_active);

CREATE INDEX idx_classification_history_account ON account_classification_history(account_mapping_id);
CREATE INDEX idx_classification_history_org ON account_classification_history(organization_id);
CREATE INDEX idx_classification_history_date ON account_classification_history(changed_at);
...
INSERT INTO account_categorization_rules (
    rule_name,
    priority,
    conditions,
    actions,
...
-- Global rules (organization_id = NULL for system-wide rules)
(

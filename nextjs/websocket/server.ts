...
wss.on('connection', function connection(ws: WebSocket) {
  ...
  customWs.on('message', function message(data: string) {
    // Explicitly type data as string
    ...
    try {
      ...
      switch (parsedMessage.type) {
        case 'auth':
          ...
          console.log(
            `Client authenticated: User ${customWs.userId}, Project ${customWs.projectId}`
          );
        ...
        case 'project_update':
          ...
          wss.clients.forEach((client) => {
            ...
            if (
              ...
              clientWs.projectId === parsedMessage.payload.projectId
            ) {
              clientWs.send(
                JSON.stringify({ type: 'project_updated', payload: parsedMessage.payload })
              );
            }
          });
          console.log(
            `Project ${parsedMessage.payload.projectId} updated:`,
        ...
        case 'document_analysis_progress':
          ...
          wss.clients.forEach((client) => {
            ...
            if (
              clientWs.readyState === WebSocket.OPEN &&
              clientWs.userId === parsedMessage.payload.userId
            ) {
              clientWs.send(
                JSON.stringify({ type: 'analysis_progress', payload: parsedMessage.payload })
              );
            }
          });
          console.log(
            `Analysis progress for user ${parsedMessage.payload.userId}: ${parsedMessage.payload.progress}%`
          );
          break;
        default:

...
export class ConfigManager {
  ...
  constructor(options: ConfigManagerOptions = {}) {
    ...
    this.geminiCircuitBreaker = new CircuitBreaker(3, 60000, 2);
  }
    ...
    try {
      ...
      switch (integration.type) {
        case IntegrationType.GEMINI:
          testResult = await this.testGemini(integration.endpoint, apiKey, integration.config);
          break;
  ...
  private async testGemini(
    ...
    return await this.geminiCircuitBreaker.execute(async () => {
      const start = Date.now();
      // This is a placeholder for a real Gemini API test
      // You would typically make a simple API call to check connectivity and authentication
      const url = `${endpoint}`;
   ...
   */
  async getGeminiConfig(organizationId?: string): Promise<Record<string, any>> {
    await this.ensureInitialized();

    try {
      // Get Gemini integration
      const integrations = await this.getIntegrationsByType(IntegrationType.GEMINI, organizationId);
      ...
      if (error instanceof Error) {
        this.logger.error('Failed to get Gemini configuration', error);
      } else {
        this.logger.error('Failed to get Gemini configuration', new Error(String(error)));
      }
      // Fallback to environment variables
      return {
        apiKey: process.env.GEMINI_API_KEY,

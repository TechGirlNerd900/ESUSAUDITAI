...
export class AzureServices {
  ...
  async uploadToBlob(fileName: string, fileBuffer: Buffer, contentType: string): Promise<UploadResult> {
  ...
  async analyzeDocument(documentUrl: string, documentType: string = 'prebuilt-document'): Promise<ExtractedDocument> {

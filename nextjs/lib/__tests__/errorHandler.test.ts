import { withErrorHandling, ApiError, NotFoundError, AuthorizationError, ValidationError, RateLimitError } from '../errorHandler'
...
describe('errorHandler', () => {
  ...
  const mockHandler = async (req: NextRequest) => {
    ...
    switch (action) {
      ...
      case 'validation_error':
        ...
        throw new ValidationError('Invalid input', [{ field: 'name', message: 'Name is required' }])
  ...
  it('should handle ValidationError and return 400 status with details', async () => {
    ...
    expect(data).toEqual({ error: 'Invalid input', details: [{ field: 'name', message: 'Name is required' }] })
  })

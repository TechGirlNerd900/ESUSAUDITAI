...
export default function DocumentsList({
  ...
  if (!documents.length) {
    return <div className="text-center py-8 text-gray-500">No documents uploaded yet</div>;
  }
  ...
  return (
    <div>
        ...
        {documents.map((doc) => (
          ...
          >
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                onClick={() =>
                  doc.status === 'analyzed' ? setSelectedDoc(doc) : handleAnalyze(doc)
                }
            ...
            {error && (
              <div className="absolute left-0 bottom-0 text-xs text-red-500 mt-1">{error}</div>
            )}
          </div>
        ))}
      </div>
      ...
      {selectedDoc && (
        <DocumentAnalysisModal document={selectedDoc} onClose={closeModal} projectId={projectId} />
      )}

  ...
  try {
    ...
    if (!auth.success) {
      return NextResponse.json({
        ...
        errorResponse: auth.success === false && auth.error ? {
          status: auth.error.status,
          statusText: auth.error.message
        } : null
      })
    }
    ...
    return NextResponse.json({
      ...
      diagnostics: {
        ...
        inactiveProfile: auth.profile && (!auth.profile.is_active || auth.profile.status !== 'active'),

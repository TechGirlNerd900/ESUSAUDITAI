...
-- Policy: Users can update compliance assessments they created
...
-- Policy: Users can delete compliance assessments they created
...
LEFT JOIN profiles pr ON ca.created_by = pr.id
...
-- Add RLS to the view
ALTER VIEW compliance_assessment_summary ENABLE ROW LEVEL SECURITY;

-- Policy for compliance assessment summary view
CREATE POLICY "Users can view compliance assessment summary from their organization" ON compliance_assessment_summary
    FOR SELECT USING (
        organization_id IN (
            SELECT organization_id FROM profiles WHERE id = auth.uid()
        )
